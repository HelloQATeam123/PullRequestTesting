...
Shopping Time is a modern, full-featured e-commerce web application built with React. The platform showcases over 150 women's clothing products with comprehensive customization options including size and color variations, dynamic cart management, and multi-currency support.
...
### Cart Management
- **Add/Remove Products**: Seamlessly add products to your shopping cart or remove them as needed
- **Quantity Adjustment**: Modify product quantities directly from the cart interface
- **Cart Overlay**: Quick-access cart summary overlay for efficient shopping experience
...
- **Dynamic Currency Switching**: Change store currency on-the-fly to match your preference

### Checkout Process
- **Multi-Step Checkout**: Intuitive, guided checkout flow for secure transactions
...
### `npm run build`
Creates an optimized production build in the `build` folder. The build is minified, and filenames include content hashes for efficient caching.

### `npm run eject`
**Warning: This is a one-way operation!**

Ejects the application from Create React App, providing full control over configuration files and dependencies. Only use if you need complete customization beyond what Create React App provides.
